# U09_Ex01_RacquetballBestOfNGames.py
#
#  Author: Bill Montana
...
#   First service alternates: A in odd games, B in even games
#
...
def simNMatches(m, g, probA, probB):
    ...
    for match in range(m):
        # simulate a match
        gamesA, gamesB = simOneMatch(g, probA, probB, 'B' if match % 2 == 0 else 'A')
...
def simOneMatch(g, probA, probB, server):
    '''
    ...
    :param probA: float -> probability player A wins serve
    :param probB: float -> probability player B wins serve
    :param server: str -> server of first game
